...
def _at(month, day, hour, minute):
    """UTC moment."""
    return datetime(2026, 1, day, hour, minute, tzinfo=timezone.utc) if month == 1 else None


def test_approx_open_midday_weekday():
    svc = MarketHoursService(client=NOCRED)
    # Wed Jan 7 2026, 17:00 UTC = 12:00 ET → open
    info = svc.phase(datetime(2026, 1, 7, 17, 0, tzinfo=timezone.utc))
...
def test_approx_pre_market():
    ...
    info = svc.phase(datetime(2026, 1, 7, 13, 0, tzinfo=timezone.utc))
...
def test_approx_post_market():
    ...
    info = svc.phase(datetime(2026, 1, 7, 21, 30, tzinfo=timezone.utc))
...
def test_approx_weekend_closed_next_open_is_monday():
    ...
    info = svc.phase(datetime(2026, 1, 10, 15, 0, tzinfo=timezone.utc))
...
def test_dst_winter_vs_summer_session_bounds():
    ...
    winter = svc.phase(datetime(2026, 1, 7, 14, 30, tzinfo=timezone.utc))
    summer = svc.phase(datetime(2026, 7, 8, 14, 30, tzinfo=timezone.utc))
...
def test_effective_data_end_open_market_is_now():
    svc = MarketHoursService(client=NOCRED)
    moment = datetime(2026, 1, 7, 17, 0, tzinfo=timezone.utc)  # open
...
def test_effective_data_end_closed_market_snaps_to_session_close():
    ...
    end = svc.effective_data_end(datetime(2026, 1, 10, 15, 0, tzinfo=timezone.utc))
